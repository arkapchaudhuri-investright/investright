...
def test_paste_mixed_good_blank_garbage():
    ...
    assert rows == [
        {"raw_symbol": "AAPL", "isin": "", "qty": 10.0, "avg_price": 150.0},
        {"raw_symbol": "MSFT", "isin": "", "qty": 5.0, "avg_price": 300.0},
    ]


def test_paste_strips_currency_and_thousands_commas():
    rows = p.parse_rows("RELIANCE.NS 25 ₹2,450.50\nTSLA $3 $1,200", "paste")
    assert rows[0] == {"raw_symbol": "RELIANCE.NS", "isin": "", "qty": 25.0, "avg_price": 2450.50}
    assert rows[1] == {"raw_symbol": "TSLA", "isin": "", "qty": 3.0, "avg_price": 1200.0}


def test_paste_skips_header_line():
    rows = p.parse_rows("Symbol Qty Price\nAAPL 2 100", "paste")
    assert rows == [{"raw_symbol": "AAPL", "isin": "", "qty": 2.0, "avg_price": 100.0}]
...
def test_generic_csv_explicit_colmap():
    ...
    assert rows == [{"raw_symbol": "AAPL", "isin": "", "qty": 10.0, "avg_price": 150.0}]
...
def test_broker_zerodha_signature():
    ...
    assert rows == [
        {"raw_symbol": "RELIANCE", "isin": "", "qty": 25.0, "avg_price": 2450.50},
        {"raw_symbol": "TCS", "isin": "", "qty": 10.0, "avg_price": 3200.0},
    ]
...
def test_broker_miss_falls_back_to_guess_not_error():
    ...
    rows = p.parse_rows(csv, "zerodha")
    assert rows == [{"raw_symbol": "AAPL", "isin": "", "qty": 4.0, "avg_price": 120.0}]
...
def test_bad_numbers_kept_as_none_for_csv():
    ...
    csv = "Ticker,Shares,Avg Cost\nAAPL,,\n"
    rows = p.parse_rows(csv, "generic")
    assert rows == [{"raw_symbol": "AAPL", "isin": "", "qty": None, "avg_price": None}]
